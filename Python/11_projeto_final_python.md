...
A aplicação deve ser um jogo de perguntas e respostas, onde o utilizador responde a 10 perguntas carregadas de um ficheiro JSON. O programa deve ter um menu para iniciar o jogo, mostrar regras, e sair. Durante o jogo, o utilizador responde a perguntas, ganha pontos por respostas corretas, e no final vê um resumo da sua pontuação.
...
Antes de escrever código “a sério”, o grupo tem de entregar um ficheiro chamado:
...
#### 3) Estrutura de ficheiros / módulos (decidida por vocês)
...
#### 4) Lista de funções (com responsabilidades)
...
| Função               | Descrição                                 | Responsabilidade | Localização (nome do ficheiro) |
| -------------------- | ----------------------------------------- | ---------------- | ------------------------------ |
| `carregar_perguntas` | Leitura do ficheiro JSON com as perguntas | João             | `perguntas.py`                 |
| `mostrar_menu`       | Mostra o menu e lê escolha do utilizador  | Maria            | `menus.py`                     |
| etc...               | ...                                       | ...              | ...                            |
...
Uma aplicação é composta por vários “ecrãs” ou “estados” (menu, jogo, resumo, etc.). Usando o fluxo que desenharam no ponto anterior, vão definir uma sequência de ecrãs do programa, e o que cada um tem de mostrar.

Por exemplo:

| Ecrã    | O que mostrar / pedir ao utilizador               |
| ------- | ------------------------------------------------- |
| Menu    | Opções numeradas: (1) Jogar, (2) Regras, (3) Sair |
| Regras  | Texto com as regras do jogo e como jogar          |
| etc.... | ..                                                |

#### 7) Plano de testes (manual)
...
#### 8) Mini-revisões antes de começar a programar
...
- Funções para carregar e salvar ficheiros JSON
- Funções para criar a parte visual de cada ecrã (menu, perguntas, resumo):
    - Cada ecrã deverá ser uma função que mostra o que tem de mostrar e devolve a resposta do utilizador (se for o caso)
      Por exemplo:
    ```python
    def mostrar_menu():
...
- Validar as entradas do menu e do jogo (não crashar com inputs inválidos: letras em vez de números, números fora do intervalo, etc.)
